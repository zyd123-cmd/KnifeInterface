...
@router.get("/lend-records", response_model=LendRecordResponse, tags=["领刀记录"])
async def get_auditor_lend_records(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取领刀记录失败: {str(e)}")


@router.get("/lend-records/export", tags=["领刀记录"])
...
@router.post("/restock", response_model=RestockResponse, tags=["领刀记录"])
async def auditor_restock_cabinet(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"补货失败: {str(e)}")


@router.get("/replenish-records", response_model=ReplenishRecordResponse, tags=["领刀记录"])
async def get_auditor_replenish_records(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取补货记录失败: {str(e)}")


@router.get("/storage-records", response_model=StorageRecordResponse, tags=["领刀记录"])
async def get_auditor_storage_records(
    current: Optional[int] = Query(None, description="当前页"),
    end_time: Optional[str] = Query(None, description="结束时间"),
    order: Optional[int] = Query(None, description="顺序 0: 从大到小 1：从小到大"),
    ranking_type: Optional[int] = Query(None, description="0: 数量 1: 金额"),
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取公共暂存记录失败: {str(e)}")


@router.get("/personal-storage", response_model=PersonalStorageResponse, tags=["领刀记录"])
async def get_auditor_personal_storage(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取个人暂存柜信息失败: {str(e)}")


@router.get("/make-alarm", response_model=MakeAlarmResponse, tags=["领刀记录"])
async def set_auditor_make_alarm(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"设置告警值失败: {str(e)}")


@router.get("/get-make-alarm", response_model=CabinetAlarmResponse, tags=["领刀记录"])
async def get_auditor_make_alarm(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取告警值失败: {str(e)}")
